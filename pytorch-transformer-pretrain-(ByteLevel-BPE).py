...
CONFIG = {
    ...
    'pretrain_epochs': 100,               # Number of pre-training epochs
    ...
    'max_pretrain_sentences': 1000,    # Maximum sentences for pre-training (for quick test)
    'max_translation_pairs': 50,      # Maximum translation pairs for fine-tuning (for quick test)
    'mask_probability': 0.15,            # Probability of masking tokens during pre-training
    'device': 'cuda' if torch.cuda.is_available() else 'cpu',  # Use GPU if available
    'max_train_minutes': 20,           # Maximum allowed training time (in minutes) for each phase
    'max_global_minutes': 45,          # Maximum allowed total wall-clock time (in minutes) for the whole script
...
class PretrainDataset(Dataset):
    ...
    def mask_tokens(self, tokens):
        ...
        for i, token in enumerate(tokens):
            if token in [0, 2, 3, 4]:  # Don't mask PAD, SOS, EOS, MASK
                continue
    ...
    def __getitem__(self, idx):
        ...
        sos = self.vocab.get_vocab().get('<SOS>', 2)
        eos = self.vocab.get_vocab().get('<EOS>', 3)
        tokens = [sos] + tokens + [eos]
        masked_tokens, labels = self.mask_tokens(tokens)
        # Pad sequences
        pad = self.vocab.get_vocab().get('<PAD>', 0)
...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tgt_encoded_ip = [self.tgt_vocab.get_vocab().get('<SOS>', 2)] + self.tgt_vocab.encode(tgt_sentence)
        tgt_encoded_op = self.tgt_vocab.encode(tgt_sentence) + [self.tgt_vocab.get_vocab().get('<EOS>', 3)]
        # Truncate if too long
        ...
        # Pad sequences
        pad = self.src_vocab.get_vocab().get('<PAD>', 0)
...
def pretrain_model(model, english_sentences, bengali_sentences, src_vocab, tgt_vocab, config, overall_start_time=None):
    ...
    optimizer = optim.Adam(model.parameters(), lr=config['pretrain_learning_rate'], betas=(0.9, 0.98), eps=1e-9)
...
class TranslationInference:
    ...
    def translate(self, sentence, max_length=None):
        ...
        tgt_input = [self.tgt_vocab.get_vocab().get('<SOS>', 2)]
        with torch.no_grad():
            for _ in range(max_length):
                ...
                if next_token == self.tgt_vocab.get_vocab().get('<EOS>', 3):
                    break
        # Decode the result (excluding SOS and EOS tokens)
        result_tokens = tgt_input[1:-1] if tgt_input[-1] == self.tgt_vocab.get_vocab().get('<EOS>', 3) else tgt_input[1:]
...
class BPETokenizer:
    ...
    def __init__(self, vocab_size=10000, language="mixed"):
        ...
        self.tokenizer = None
        self.language = language  # "english", "bengali", or "mixed"
    ...
    def train(self, sentences):
        ...
        # Prepare a temporary file with one sentence per line
        ...
        try:
            ...
            # Set up normalization based on language
            ...
            # Set up pre-tokenization (ByteLevel preserves spaces)
            ...
            trainer = trainers.BpeTrainer(
                ...
                min_frequency=1,  # Minimum frequency for subword units
                show_progress=True
            )
            ...
            # Set up decoder (ByteLevel for space preservation)
            tokenizer.decoder = decoders.ByteLevel()
            
            # Set up post-processor to handle special tokens properly
    ...
    def get_special_token_id(self, token, tokenizer):
        ...
        return vocab.get(token, vocab.get("<UNK>", 1))
    ...
    def decode(self, ids, skip_special_tokens=True):
        ...
        if skip_special_tokens:
            ...
            # Remove None values and filter IDs
...
def main():
    ...
    if CONFIG.get('shared_bpe_vocab', False):
        ...
        bpe_tokenizer_bn.train(bengali_sentences)
        print("Bengali BPE vocab size:", len(bpe_tokenizer_bn.get_vocab()))

        src_vocab = bpe_tokenizer_en
        tgt_vocab = bpe_tokenizer_bn

    # 3. Create model
    print("Creating transformer model...")
    model = Transformer(
        src_vocab_size=len(src_vocab.get_vocab()),
        tgt_vocab_size=len(tgt_vocab.get_vocab()),
